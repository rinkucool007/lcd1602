...
# Change text to anything you wish, but must be 16 characters or less
  ...
  while True:
    lcd_text(str(datetime.today().strftime("%I:%M %p")),LCD_LINE_1)
    lcd_text(str(datetime.today().strftime('%A')),LCD_LINE_2)
    time.sleep(3) # 3 second delay

    lcd_text(str(datetime.today().strftime("%I:%M %p")),LCD_LINE_1)
    lcd_text(str(date.today()),LCD_LINE_2)
    time.sleep(3) # 3 second delay
...
try:
  ...
  lcd_text("Have a Great Day",LCD_LINE_2)
  GPIO.cleanup()
